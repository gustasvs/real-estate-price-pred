    ...
    for tr in soup.find_all("tr"):
        ...
        if len(tds) == 2:
            ...
            if label_for_comparison in keywords:
                property_metadata[label_for_comparison] = process_property_value(
                    label_for_comparison, value
                )
...
def download_image(property_url, image_url):
    ...
    finalised_image_path = (
        f"{property_url.split('/')[-1].split('.')[0]}-{image_url.split('/')[-1]}"
    )
    file_extension = (
        image_url.split(".")[-1] if "." in image_url.split("/")[-1] else "jpg"
    )

    processed_image = process_image(response.content)
    processed_image.save(
        f"images/{finalised_image_path}", format=get_correct_format(file_extension)
    )
...
def check_if_already_handled(url):
    ...
    if url_in_csv:
        ...
        url_images_processed = any(
            file.startswith(url_identifier) for file in image_files
        )

    return (
        url_in_csv or url_images_processed,
        f"{url_in_csv and 'URL already in CSV.'} {url_images_processed and 'Images already processed.'}",
    )
    ...
    try:
        ...
        for url in pbar:
            ...
            pbar.set_description(
                f"{url.split('/')[-1]} Images: {len(storage_paths_for_images)}"
            )
