...
from config.settings import (
    ...
    LOSS_FUNCT,
    EPOCHS,
    BATCHES_TO_AGGREGATE,
    WEIGHTED_BIN_COUNT,
    SAMPLES_TO_USE,
    USE_ADDITIONAL_METADATA,
)
...
def train_epoch(model, dataloader, optimizer, device, epoch):
    ...
    for mini_batch_idx, (sample, prices) in enumerate(dataloader, 1):
        ...
        if ENABLE_DEV_LOGS:
            print(
                "one input in batch shape: ", sample[0].shape
            )  # Should print [8, 2, 3, 224, 224] for a batch size of 8
        ...
        if (
            mini_batch_idx + 1
        ) % BATCHES_TO_AGGREGATE == 0 or mini_batch_idx == total_batches:
...
for i, aggregation_method in enumerate(aggregation_methods):
    for j, (model_name, model_info) in enumerate(models.items()):
        ...
        actual_model_name = (
            model_name.replace(" head-only", "") if train_only_head else model_name
        )
        embedding_size = model_info["embedding_size"]

        print(
            f"Training model: {actual_model_name}, aggregation method: {aggregation_method}, train_only_head: {train_only_head}"
        )
        ...
        val_loss = train_loop(
            EPOCHS, model, train_loader, val_loader, optimizer, device
        )
    ...
    if not USE_ADDITIONAL_METADATA:
        ...
        with torch.no_grad():
            for pixel_values_list, prices in val_loader:
                ...
                for batch_idx, (batch, predicted, actual) in enumerate(
                    zip(pixel_values_list, outputs, prices)
                ):
                    n_images = len(batch)
                    ...
                    for idx, img_tensor in enumerate(batch):
                        ...
                        plt.axis("off")
                    plt.suptitle(
                        f"Sample {batch_idx + 1}: Predicted: ${predicted.item():.2f}, Actual: ${actual.item():.2f}",
                        fontsize=16,
                    )
                    plt.show()
    else:
        ...
        with torch.no_grad():
            for sample, prices in val_loader:
                ...
                for batch_idx, (instances, predicted, actual) in enumerate(
                    ...
                    n_images = len(
                        instances[0]
                    )  # Assuming instances[0] contains the image tensors
                    plt.figure(figsize=(n_images * 5, 5))
